...
public struct ParticleSystem : IJobParallelFor
{
    ...
    void CalculateParticleForces()
    {
        ...
        float maxParticlesToSimulate = Settings.MaxParticlesToSimulate;
        float constantMult = Settings.ConstMult;
        
        #region collision and force calc
        for (int i = 0; i < particles.Length; i++)
        {
            var p1Pos = particles[i];
            ...
            for (int j = 0; j < queryResults.Length; j++)
            {
                ...
                if (p1P2Dist < maxRadius)
                {
                    ...
                    float scale = currentToDesiredPercentage * constantMult * _forceMultiplierCurve.Evaluate(indexInCurve) * deltaTime;
                    Vector3 posToAddScaled = p1ToP2Dir * scale;
                    
                    ToMove[i] += posToAddScaled;
                }
            }
